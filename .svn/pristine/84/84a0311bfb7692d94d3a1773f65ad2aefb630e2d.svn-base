...
public class ExecutorServiceManager {
     ...
     * 发起关闭线程池命令并等待任务执行完成
     * @param exec
     * @param awaitTime 等待时间秒
     */
    public static void shutdownAndAwaitTermination(ExecutorService exec,long awaitTime){
        exec.shutdown();
        try {
            //等待任务全部完成或达到指定时间
...
//                    logger.error("exec did not terminate");
                }
            }
        } catch (InterruptedException e) {

//            logger.error("线程池等待过程中异常",e);
            exec.shutdownNow();
